//! Test of dynamic rendering pipeline with swapchain and root pointer.
...
use rust_and_vulkan::{
    SdlContext, SdlWindow, VkImageLayout, VulkanDevice, VulkanInstance, VulkanSurface,
};
...
// Set to true for automated testing (exits after 2 frames)
const TEST_MODE: bool = false;

fn main() -> Result<(), String> {
    println!("Dynamic Rendering Pipeline Test - Rotating Triangle");
    println!("===================================================");
    println!("Press ESC to exit");

    // Initialize SDL3 and Vulkan
    let sdl = SdlContext::init()?;
    let window = SdlWindow::new("Dynamic Rendering - Press ESC to exit", 800, 600)?;
    let instance = VulkanInstance::create(&sdl, &window)?;

    // Create surface
    let surface = VulkanSurface::create(&window, &instance)?;

    // Create Vulkan device (dynamic rendering extension already enabled in lib.rs)
    ...
    // Load SPIR-V shaders (same as before)
    ...
    // Create graphics pipeline using dynamic rendering (no render pass)
    println!("Creating dynamic graphics pipeline...");
    let pipeline = GraphicsPipeline::new_dynamic(
        &context,
        &vert_shader,
        &frag_shader,
        &layout,
        swapchain.format(), // color format
        None,               // no depth/stencil
    )
    .map_err(|e| format!("Failed to create dynamic graphics pipeline: {}", e))?;
    println!("Dynamic graphics pipeline created.");
    ...
    while !quit && (!TEST_MODE || frames_rendered < MAX_TEST_FRAMES) {
        ...
        cmd.transition_swapchain_image(
            swapchain_image,
            VkImageLayout::VK_IMAGE_LAYOUT_UNDEFINED,
            VkImageLayout::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        );

        // Begin dynamic rendering (instead of render pass)
        ...
        // Transition swapchain image from color attachment optimal to present src
        cmd.transition_swapchain_image(
            swapchain_image,
            VkImageLayout::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
    ...
    println!("Dynamic rendering test completed successfully.");
    Ok(())
}
